...
final class LandingPageController
{
    ...
    public function estimationLannion(): void
    {
    ...
    public function vendreMaisonLannion(): void
    {
        UtmTrackingService::capture();

        View::renderBare('landing/layout', [
            'page_title'       => 'Vendre sa Maison à Lannion | Estimation Gratuite',
            'meta_description' => 'Vous vendez votre maison à Lannion ? Obtenez une estimation gratuite et découvrez le prix de vente optimal. Sans engagement.',
            'landing_view'     => 'landing/pages/vendre-maison-lannion',
            'landing_slug'     => 'vendre-maison-lannion',
        ]);
    }
    ...
    public function avisValeurGratuit(): void
    {
        ...
        View::renderBare('landing/layout', [
            'page_title'       => 'Avis de Valeur Gratuit Lannion | Sans Engagement',
            'meta_description' => 'Recevez un avis de valeur gratuit pour votre bien immobilier à Lannion. Analyse basée sur le marché actuel. Résultat immédiat.',
    ...
    public function submitLead(): void
    {
        ...
        try {
            ...
            $ville     = trim((string) ($_POST['ville'] ?? 'Lannion'));
            if ($ville === '') {
                $ville = 'Lannion';
            }
            ...
            $landingSlug = trim((string) ($_POST['landing_slug'] ?? 'estimation-lannion'));
            $landingView = 'landing/pages/' . preg_replace('/[^a-z0-9\-]/', '', $landingSlug);

            // Check view exists, fallback to estimation-lannion
            $viewPath = __DIR__ . '/../views/' . $landingView . '.php';
            if (!is_file($viewPath)) {
                $landingView = 'landing/pages/estimation-lannion';
            }

            View::renderBare('landing/layout', [
                'page_title'       => 'Estimation Immobilière Lannion',
                'meta_description' => 'Estimation immobilière gratuite à Lannion.',
