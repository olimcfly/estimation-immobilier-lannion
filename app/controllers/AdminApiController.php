...
final class AdminApiController
{
    ...
    private function testGoogleMaps(): array
    {
        ...
        $url = 'https://maps.googleapis.com/maps/api/geocode/json?address=' . urlencode('Lannion, France') . '&key=' . urlencode($apiKey);
        ...
        $address = $data['results'][0]['formatted_address'] ?? 'Lannion';
    ...
    {
        $url = 'https://api.cquest.org/dvf?code_postal=22300&nature_mutation=Vente&limit=1';
        ...
        return [
            'success' => true,
            'message' => 'API fonctionnelle',
            'response' => 'DVF accessible - ' . ($count ?? count($features)) . ' transaction(s) trouvee(s) pour 22300',
            'latency_ms' => $latency,
        ];
    }
