...
final class AIService
{
    public function generateArticle(string $persona, string $awarenessLevel, string $topic): array
    {
        ...
        $prompt = "Tu es un copywriter immobilier expert Lannion. Rédige un article SEO en HTML strict avec les balises <h2>, <h3>, <p>, <ul>, <li>.\n"
        ...
        return [
            ...
            'meta_description' => (string) ($decoded['meta_description'] ?? 'Découvrez notre analyse du marché immobilier à Lannion.'),
    ...
    private function fetchPerplexityTrends(string $topic, string $persona): string
    {
        ...
        $prompt = sprintf(
            'Résume en 6 points les tendances immobilières vendeurs à Lannion pour le sujet "%s" et le persona "%s".',
            $topic,
            $persona
        );
    ...
    private function fallbackArticle(string $persona, string $awarenessLevel, string $topic, string $trendInsights): array
    {
        ...
        return [
            'title' => $title,
            'meta_title' => $title . ' | Blog Immobilier Lannion',
            'meta_description' => 'Conseils concrets pour vendre votre bien à Lannion selon votre situation.',
